...
static void pow_forward(Node &node) {
  ...
  node.dval =
      node._children[1]->val * std::pow(node._children[0]->val, node._children[1]->val - 1) * node._children[0]->dval
      + node._children[1]->dval * std::pow(node._children[0]->val, node._children[1]->val) * std::log(node._children[0]->val);
}
...
static void sqrt_forward(Node &node) {
  ...
  node.dval = 0.5f * std::pow(node._children[0]->val, -0.5f) * node._children[0]->dval;
}
